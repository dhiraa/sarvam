...
_feature_type = RawWavAudio

DATADIR = '../data/tensorflow_speech_recoginition_challenge/' # unzipped train and test data
...
POSSIBLE_LABELS = 'yes no up down left right on off stop go _silence_ unknown'.split()
...
def data_generator(data, params, mode='train'):
    def generator():
        ...
        for i, (label_id, uid, fname) in enumerate(data):
            try:
                ...
                for _ in range(samples_per_file):
                    ...
                    yield dict( command_ids=np.int32(label_id),
                    raw_audio_data=wav)

                    # yield {
                    #     _feature_type.TARGET :np.int32(label_id),
                    #     _feature_type.FEATURE_1 : wav,
                    # }
...
def model_handler(features, labels, mode, params, config):
    ...
    wav = features[_feature_type.FEATURE_1]
...
train_input_fn = generator_input_fn(
    ...
    target_key=_feature_type.TARGET,  # you could leave target_key in features, so labels in model_handler will be empty
...
val_input_fn = generator_input_fn(
    ...
    target_key=_feature_type.TARGET,
    batch_size=hparams.batch_size,
    shuffle=True,
    num_epochs=None,
    queue_capacity=3 * hparams.batch_size + 10,
...
cfg, model = ModelsFactory.get("simple_conv")
