...
class TextDataset():
    ...
    def lines_to_words(self, list_of_tokenized_lines):
        ...
        words = []
        for list_tokens in tqdm(list_of_tokenized_lines, desc="lines2words"):
    ...
    def generate_raw_skip_gram_samples(self, words):
        ...
        for index, center in tqdm(enumerate(words), desc="skip_gram_samples"):
            # """ Form training pairs according to the skip-gram model. """
            context = random.randint(1, self.window_size)
            # get a random target before the center word
            for target in words[max(0, index - context): index]:
                if center in self.word_to_id and target in self.word_to_id:
    ...
    #================================================================================================
    #  Glove Word-Count Pair Generator
    #================================================================================================
    ...
    def fit_to_corpus(self, list_of_tokenized_lines):
        ...
        self.vocab = [word for word, count in tqdm(word_counts.most_common(self.vocabulary_size), desc="vocab")
                      if count >= self.min_occurrences]

        self.word_to_id = {word: i for i, word in enumerate(self.vocab)} #For faster lookup

        self.cooccurrence_matrix = {
            # (self.word_to_id[words[0]], self.word_to_id[words[1]]): count
            (word_tuple[0], word_tuple[1]): count
            for word_tuple, count in tqdm(cooccurrence_counts.items(), desc="filtering")
            if word_tuple[0] in self.word_to_id and word_tuple[1] in self.word_to_id
        }
    ...
    def get_cooccurrence_features(self, list_of_tokenized_lines):
        ...
        features = []
        for word_ids, counts in tqdm(self.cooccurrence_matrix.items(), desc="features"):
    ...
    def read_from_files(self, text_dir):
        ...
        lines = [x.strip() for filename in tqdm(file_names, desc="files")
                 for x in codecs.open(filename, "r", "utf-8").readlines()]
    ...
    def prepare(self):
        ...
        # self.number_examples = cooccurrence_features.shape[0] #to calculate number steps for trianing

        data["focal_words"] = np.array(cooccurrence_features[:,0])
        data["context_words"] = np.array(cooccurrence_features[:, 1])
        data["cooocurrence_count"] = np.array(cooccurrence_features[:, 2])

        with open("tmp/glove_dataset.pickle", "wb") as file:
        ...
        with open("tmp/skip_gram_dataset.pickle", "wb") as file:
            pickle.dump(data, file)
